...
class QuizListSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Quiz
		fields = ["id", "name", "description", "questions_count","duration","time_start","time_end"]
...
class UsersAnswerSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = UsersAnswer
		fields = ["quiz_taker","question","answer"]
...
class MyQuizListSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Quiz
		fields = ["id", "name","description", "questions_count", "completed", "score", "progress","duration","time_start","time_end"]
		# read_only_fields = ["questions_count", "completed", "progress"]

	def get_completed(self, obj):
		try:
			quiztaker = QuizTaker.objects.get(user=self.request.user, quiz=obj)
